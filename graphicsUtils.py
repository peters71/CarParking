...
def polygon(coords, outlineColor, fillColor=None, filled=1, smoothed=0, behind=0, width=1):
  c = []
  for coord in coords:
    c.append(coord[0])
    c.append(coord[1])
...
def moVe_to(object, (x1, y1), (x2, y2), (x3, y3), (x4, y4), (x5, y5),
            d_o_e=Tkinter.tkinter.dooneevent,
            d_w=Tkinter.tkinter.DONT_WAIT):
        
    horiz = True
    newCoords = [x1, y1, x2, y2, x3, y3, x4, y4, x5, y5]    
    _canvas.coords(object, *newCoords)
    d_o_e(d_w)
